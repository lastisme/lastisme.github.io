/* Build fingerprint: c90a65583515e46bc844de149f512a50 */
//GLOBAL CONSTANTS
...
function refresh() {
  ...
  contractsPromises = getContractsPromises(initPromise);
      ...
      for (var j = 0; j < contractsPromises.length; j++) {
        ...
        current.then( function(data) {
          ...
          if(noLast) {
            ...
            if(blocksRemaining==0) {
              winner.html("won " + '<span class="icon mif-dollar2 v-align-bottom"></span> ' + '<span class="v-align-bottom">' +ndata.jackpot.dollar.toFixed(2) + '</span>' );
          ...
          if(isLocal(web3) && !attached) {
            ...
            c.TicketBought(function(err,data) {
              $.Notify({ content: "Ticket bought!", style: { background: bcolor, color : fcolor } });
              refresh();
            });
