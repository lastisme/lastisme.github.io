...
function loadTable(contract) {
  ...
  Promise.all(promises).then(function(data) {
    ...
    table.empty();
    if(data.length==0) {
      table.append('<tr><td>No winners yet</td></tr>');
    }
...
function getStorageAtPromise(web3,contract,addr) {
  return new Promise(function(resolve,reject) {
    web3.eth.getStorageAt(ADDRESS, addr, 'latest', function(err,data) {
      resolve(data);
    });
  });
}
    ...
    contractsAndStats.then( function(dataArray) {
      ...
      for (var j = 0; j < contractsPromises.length; j++) {
        ...
        current.then( function(data) {
          ...
          var noLast = (new BigNumber(ndata.lastPlayer,16).toNumber()==0);
          if(noLast) {
            element.hide();
